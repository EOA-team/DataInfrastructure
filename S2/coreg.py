...
def coreg(ds, ref):
  ...
  projection_ref = ref.rio.crs.to_wkt()
...
def split_and_save(ds, minx, maxy, output_folder):
    """
    ...
    ds = ds.sel(lat=slice(minx, minx+1280), lon=slice(maxy, maxy-1280))
    
    for yr in np.arange(2017, 2024, 1):
        ds_yr = ds.sel(time=slice(f'{yr}-01-01', f'{yr}-12-31'))

        time_min, time_max = ds_year.time.min().values, ds_year.time.max().values
...
def run_coregistration(target_folder, reference_folder, output_folder):
  ...
  for i, f in enumerate(target_files):
    ...
    ds, minx, maxy = load_cubes(f, target_folder)
    # Load SwissImage of correspodnong central cube
    ref = xr.open_zarr(os.path.join(reference_folder, f'SwissImage0.1_{int(minx)}_{int(maxy)}.zarr'))    
    ...
    # Save year by year
    split_and_save(ds_coreg, minx, maxy, output_folder)
