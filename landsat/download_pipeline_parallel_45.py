...
def save_cube(cube, n_cells, output_prefix, patch_size=128, resolution=30, overwrite=False):
    """
    ...
    for i in range(n_cells):
        for j in range(n_cells):
            ...
            output_path = output_prefix + f'LS_{int(lon_start)}_{int(lat_start)}_{year_start}{month_start}{day_start}_{year_end}{month_end}{day_end}.zarr'
...
def download_year(year, specs, n_cells, output_prefix, overwrite, mega_patch):
    ...
    try:
        ...
        print(f'Download {year} took {end-start}s')
...
def run_download(grid, grid_copy, num_cells, patch_size, output_prefix, overwrite, specs):
        ...
        for i, row in grid.iterrows(): #for i, row in grid.iterrows():
            if not grid_copy.loc[i, 'selected']:
                ...
                years_to_download = list(set(range(1982, 2013)) - set(grid_copy.loc[i, 'years_done']))
                ...
                while years_to_download:
                    ...
                    with lock:
                        ...
                        grid_copy.to_pickle(output_prefix + 'grid_landsat_45.pkl')

                
                    # Mark the selected cells
                    if len(successful_years) == (2012-1982+1):
...
if __name__ == "__main__":
    ...
    if len(df_zarr):
        ...
        grid['selected'] = grid['years_done'].apply(lambda x: False if len(set(x)) < (2013-1982) else True) # years 1982 to 2012 included
        #grid['selected'] = grid['years_done'].apply(lambda x: False if len(set(x)-{1982, 1983, 2012}) < (2013-1982)-3 else True) # ignore 1982, 1983, 2012
    else:
